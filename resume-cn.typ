...
= #redact(alter: "roife")[#resume_name]
  ...
  == #fa[#project-diagram] 项目开发
    ...
    #cventry(
        ...
        - #ghrepo("roife/mole") (Verilog / MIPS) 五级流水线 CPU，完成了 *50+* 条指令及*旁路*\/*停顿*机制；实现了协处理器 CP0 以响应*中断*\/*异常*；
  ...
  == #fa[#laptop.code] 专业技能
    ...
    #grid(
        ...
        [*程序语言理论*], [#list(marker: [‣],
            [了解类型论、形式语义、形式语言与自动机、可计算性等理论；了解形式验证，学习过 Coq、Agda 等定理证明器；],
        ...
        [*编译器设计*], [*3 年经验*，熟悉编译器从语法解析到代码生成的*全 pipeline 开发*，尤其了解*编译优化*：
            #list(marker: [‣],
                ...
                [熟悉多种*中间代码表示*（SSA, MLIR, CPS 等）、各阶段*编译分析优化*（Mem2Reg、GVN、SCEV、寄存器分配等）],
                [熟悉 LLVM 的架构和 LLVM-IR，阅读过许多模块的代码，开发过多个分析和优化 pass；],
            )],
        [*程序分析*], [熟悉常见*静态分析*算法（数据流分析、控制流分析、IFDS、采用不同敏感度的*指针分析*等）],
        ...
        [*应用开发*], [
            #list(marker: [‣],
                [了解 Vue.js，熟悉 Ruby on Rails, Django 等 Web 后端框架的使用；了解使用 SwiftUI 的 iOS 应用开发；],
                [熟悉 PostgreSQL、Redis 等数据库的使用和数据库设计；熟悉 Docker 等工具和 CI/CD 配置等 DevOps 工作；],
            )],
