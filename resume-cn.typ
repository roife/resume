...
= #redact(alter: "roife")[#resume_name]
  ...
  == #fa[#graduation-cap] 教育背景
    ...
    #cventry(
        ...
        本科，计算机科学与技术｜GPA: 3.84/4.00，获得推荐免试研究生资格。 #linebreak()
        助教工作：*程序设计基础*（2020 秋），*面向对象设计与构建*（2021 秋，2022 春｜#link("https://scse.buaa.edu.cn/bkspy/bkspy/S_T_A_Rjftdbps_.htm")[S.T.A.R. 团队]，负责课程设计和系统开发运维）
    ]
  ...
  == #fa[#briefcase] 工作经历
    ...
    #cventry(
        ...
        - 参与统一 NVIDIA GPU 图形编译器与 NVVM 的访存指令向量化器，使得图形编译器的向量化器与 LLVM 上游保持一致：
          - 基于 LLVM GPU 访存向量化器的核心算法设计了*多地址图形访存指令*的编码方案，为多条图形访存指令实现了引用分析和向量化，同时确保尽可能减小代码库与上游代码的差异，减小后续维护成本；
    ...
    #cventry(
        ...
        - 作为 #link("https://www.rust-lang.org/governance/teams/compiler#team-rust-analyzer-contributors", [*Rust-lang Organization 成员*])（rust-analyzer-contributors-team） 和 *rust-analyzer 维护者*（Rust 语言官方 IDE）之一，社区中贡献排名在*前 1%*，参与 issues 处理、会议讨论、PR 审核等维护工作；
        ...
        - 参与维护 rust 语言社区其他项目，如 rust-clippy 等；
    ]

  == #fa[#trophy] 奖项荣誉

  - 2022 年本科生*国家奖学金*（该学年排名 1/195）；北京航空航天大学优秀毕业生；
  ...
  == #fa[#project-diagram] 项目开发
    ...
    #cventry(
        ...
        br: [Rust],
        tr: [#ghrepo("roife/ailurus", icon: true)],
    )[
    ...
    #cventry(
        ...
        - *#fa[#rust]* 负责维护官方 IDE #ghrepo("rust-lang/rust-analyzer", icon: true)；在 rust 社区也贡献过 #ghrepo("rust-lang/rust", icon: true)，#ghrepo("rust-lang/rust-clippy", icon: true)，#ghrepo("rust-lang/rustup", icon: true)，#ghrepo("rust-lang/rust-mode", icon: true) 等；
        - #ghrepo("llvm/llvm-project", icon: true)，#ghrepo("clangd/vscode-clangd", icon: true)，#ghrepo("zed-industries/zed", icon: true)，#ghrepo("MikePopoloski/slang", icon: true), #ghrepo("google/autocxx", icon: true)，#ghrepo("yuin/goldmark", icon: true), #ghrepo("moonbitlang/tree-sitter-moonbit", icon: true)，更多项目见 #link("https://github.com/roife")[GitHub]。
    ]
  ...
  == #fa[#laptop.code] 专业技能
    ...
    #grid(
        ...
        [*程序语言理论*], [#list(marker: [‣],
            [了解类型论、形式语义、形式语言与自动机、计算理论的相关知识；学习过 Coq、Agda 等定理证明器的使用；],
            [熟悉 Hindley-Milner, System F, Dependent Type 等*类型系统*的理论和实现；],
        )],
        [*程序分析*], [熟悉常见*静态分析*算法（数据流分析、CFA、IFDS、采用不同敏感度的*指针分析*等）],
        [*编译器设计*], [*3 年经验*，熟悉编译器从语法解析到代码生成的*全 pipeline 开发*，尤其了解*编译优化*：
            #list(marker: [‣],
                [了解面向对象、函数式等多种范式编程语言的实现，熟悉双向类型检查、模块系统等编程语言特性的实现；],
                [熟悉多种*中间代码表示*（SSA, MLIR, CPS 等）、各阶段*编译分析优化*（Mem2Reg、SCEV、寄存器分配等）],
                [熟悉 LLVM 的架构和 LLVM-IR，阅读过许多模块的代码，开发过多个分析和优化 pass；],
            )],
        [*IDE 开发*], [*2 年经验*，熟悉*增量计算* IDE 架构；熟悉 rust-analyzer 和 clangd 代码库；熟悉 LSP 协议和 VS Code 插件开发；],
        [*体系结构*], [了解 ARM, X86 等常见指令集的架构，了解现代处理器乱序执行、多核通信等概念，了解 NVIDIA GPU 架构；],
        [*开发环境*], [熟悉 Emacs；习惯在 macOS / Linux 下工作；能熟练使用生成式 AI 工具提高工作效率。],
    )
